...
function Codea.commit(conf, callback)
    ...
    conf.branch = conf.branch or "master"
    
    -- @todo
    -- merge codea branch to master
    ...
    local getBranch, getCommit, getTree, createTree, createCommit, updateBranchReference, fend
    
    local newBranch = false
    
    getBranch = function()
        GithubAPI.GitData.References.get({
            ref = "heads/" .. conf.branch
        }, {
            success = getCommit,
            fail = createTree
        })
    end
    getCommit = function(data)
    ...
    createTree = function(data)
        ...
        local tree
        if not data or not data.tree then -- if new branch or new project
            newBranch = true
            tree = {}
        else
            tree = data.tree
...
function Codea.pull(args, callback)
    assert(args.owner or GithubAPI.default.owner)
    GithubAPI.default.owner = args.owner or GithubAPI.default.owner
    
    assert(args.repo or GithubAPI.default.repo)
    GithubAPI.default.repo = args.repo or GithubAPI.default.repo
    
    GithubAPI.PullRequests.create({
        base = args.base or "master",
        head = args.head or "codea",
        title = args.title or "Merge Codea branch into Master",
        body = args.body or nil
    }, function(data)
        print('Pull success !')
        print(data._links.html.href)
        callback()
    end)
end

function Codea.release()
    -- local createTag
    -- local updateTagReference
    -- Release
